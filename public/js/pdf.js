...
async function desenharPaginaOferta(doc, W, H, ML, MR, TW) {
  ...
  if (qrBase64) {
    ...
    // Borda branca ao redor do QR — CENTRALIZADA
    doc.setFillColor(255, 255, 255);
...
// ── RENDERIZAR CONTEÚDO EM CARDS VISUAIS ──────────────────────────
function renderizarConteudoEmCards(doc, textoLimpo, W, H, ML, MR, TW, pageNum) {
  ...
  const desenharCard = (titulo, subtitulo, conteudo) => {
    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
    ...
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    const alturaSubtitulo = 6;
    
    const linhasConteudo = [];
    conteudo.forEach(linha => {
      const quebradas = doc.splitTextToSize(linha, TW - larguraBarra - paddingCard * 2 - 2);
      linhasConteudo.push(...quebradas);
    });
    const alturaConteudo = linhasConteudo.length * 5.5;
    
    const alturaCard = paddingCard * 2 + alturaTitulo + alturaSubtitulo + alturaConteudo + 4;
    
    if (py + alturaCard > H - MB) {
      doc.addPage();
      doc.setFillColor(255, 255, 255);
      doc.rect(0, 0, W, H, 'F');
    ...
    doc.roundedRect(ML, py, TW, alturaCard, 3, 3, 'F');
    
    doc.setFillColor(...corBarra);
    doc.rect(ML, py, larguraBarra, alturaCard, 'F');
    
    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(...corTitulo);
    doc.text(titulo, ML + larguraBarra + paddingCard, py + paddingCard + 5);
    
    doc.setFontSize(9);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(...corSubtitulo);
    doc.text(subtitulo, ML + larguraBarra + paddingCard, py + paddingCard + alturaTitulo + 3);
    
    doc.setFontSize(9);
    doc.setTextColor(...corTexto);
    let yConteudo = py + paddingCard + alturaTitulo + alturaSubtitulo + 4;
    linhasConteudo.forEach(linha => {
      doc.text(linha, ML + larguraBarra + paddingCard + 2, yConteudo);
      yConteudo += 5.5;
    });
    
    py += alturaCard + margemCard;
  };
  ...
  for (const rawL of linhas) {
    ...
    if (tipo === 'titulo') {
      if (tituloAtual) {
        desenharCard(tituloAtual, subtituloAtual, conteudoAtual);
      }
      tituloAtual = linha;
      subtituloAtual = '';
      conteudoAtual = [];
    } else if (tipo === 'paragrafo' && !subtituloAtual && tituloAtual) {
      subtituloAtual = linha.substring(0, 60) + (linha.length > 60 ? '...' : '');
    } else {
